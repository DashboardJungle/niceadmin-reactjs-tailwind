                ...
                {item.children ? (
                  <div
                    ...
                    {activeDropdown === item.id && (
                      <div
                        className={`absolute left-0 rtl:right-0 mt-2  bg-white dark:bg-black rounded-md shadow-lg ${item.column == 4 ? 'w-screen max-w-[800px]' : 'w-52'
                          }`}
                        onMouseEnter={() => handleDropdownEnter(item.id)}
                  ...
                  <Link to={item.href}>
                    <p
                      className={`py-2 px-3 rounded-md flex gap-3 items-center ${active === item.id
                        ? 'bg-error text-white'
