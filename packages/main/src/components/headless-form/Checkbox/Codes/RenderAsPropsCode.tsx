          ...
          <Field className="flex gap-3">
            <Checkbox checked={enabled} onChange={setEnabled} as={Fragment}>
              {({ checked, disabled }) => (
                ...
                >
                  <svg
                    className={clsx('stroke-white', checked ? 'opacity-100' : 'opacity-0')}
                    viewBox="0 0 14 14"
                    fill="none"
                    height={15}
                  >
            ...
            <Label>
              Each component also exposes information about its current state via render props that
              you can use to conditionally apply different styles or render different content.
            </Label>
          </Field>
        </div>
      </div>
...
export default RenderAsPropsCode;
