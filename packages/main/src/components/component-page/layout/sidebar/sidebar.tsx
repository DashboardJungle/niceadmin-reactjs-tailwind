...
import img2 from 'src/assets/images/svgs/headless.svg'
import img3 from 'src/assets/images/svgs/shadcn.svg'
import img4 from 'src/assets/images/svgs/shadcn-white.svg'
...
export default function ComponentSidebar({
  ...
  const menu: MenuItem[] =
    SidebarContent[selectedLibrary as keyof SidebarContentType] || []
  ...
  const handleLibraryChange = useCallback(
    (library: string) => {
      ...
      router(`/componentpage/ui-components/${library}/${targetComponent}`)
    },
    [onLibraryChange, router, currentComponent]
  )

  const libraries = useMemo(
    () => [
      { key: 'flowbite', label: 'Flowbite', icon: img1 },
      {
        key: 'shadcn',
        label: 'ShadCN',
        icon: img3,
  ...
  return (
    <div className='h-full rounded-md overflow-hidden bg-transparent'>
      <Sidebar className='h-full bg-transparent border-0 shadow-none'>
        ...
        {/* menu */}
        <SimpleBar className='h-[calc(100vh-280px)] lg:h-[calc(100vh-200px)]'>
