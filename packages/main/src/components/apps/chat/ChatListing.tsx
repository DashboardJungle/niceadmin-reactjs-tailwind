  ...
  return (
    <>
      <div className="left-part w-full flex flex-col h-full overflow-hidden px-0 ">
        ...
        {/* Listing */}
        <div>
          <SimpleBar className="md:h-[calc(100vh_-_450px)] h-[calc(100vh_-_200px)]">
            {filteredChats.map((chat) => (
              <div
                key={chat.id}
                className={`cursor-pointer py-4 px-6 gap-0 flex justify-between group bg-hover ${activeChatId === chat.id ? 'bg-lightprimary dark:bg-darkprimary' : 'initial'
                  }`}
