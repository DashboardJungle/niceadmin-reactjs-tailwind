...
const BlogComment = ({ comment }: BlogType | any) => {
  ...
  return (
    <>
      <div className='mt-5 p-5 bg-lightprimary dark:bg-darkprimary rounded-lg'>
        ...
        <div className='relative w-fit'>
          <Tooltip content='Reply'>
            <Button
              color={'primary'}
              className='!btn-circle !text-white flex items-center'
              onClick={() => setShowReply(!showReply)}>
              <Icon icon='solar:undo-left-linear' width={18} height={18} />
            </Button>
          </Tooltip>
        </div>
      </div>
