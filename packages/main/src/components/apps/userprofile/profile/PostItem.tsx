    ...
    <>
      <CardBox className='p-0'>
        <div className='p-6 pb-0'>
          ...
          {/**Post Content**/}
          <p className='text-black dark:text-darklink text-sm py-4'>
            {post?.data.content}
          </p>
          {/**If Post has Image**/}
          {post.data.images.length > 0 ? (
            <div className='grid grid-cols-12 gap-30'>
              {post?.data.images.map((photo, i) => {
                return (
                  <div
                    key={i}
          ...
          {post?.data.video ? (
            <iframe
              ...
              src={`https://www.youtube.com/embed/${post?.data.video}`}></iframe>
          ) : (
            ''
          )}
          ...
          <div className='flex gap-5 mt-6'>
            ...
            <div className='flex items-center gap-3 cursor-pointer text-dark font-medium text-primary-ld'>
              <Tooltip content='Comment'>
                <Button
                  className='!btn-circle !text-black p-0'
                  color={'secondary'}>
