...
const Navbar = () => {
  ...
  return (
    <>
      {announcementVisible && (
        <AnnouncementBar setIsVisible={setAnnouncementVisible} />
      )}
      <div className='landingpage'>
        <div
          className={`z-20 w-full  transition-transform duration-500 ease-in-out ${sticky
            ? 'fixed top-0'
            : announcementVisible
              ? 'absolute top-12'
              : 'absolute top-0'
              ...
              <div
                ...
                <div className='flex items-center gap-7 xl:gap-12'>
                  ...
                  <ul className='hidden lg:flex items-center gap-1'>
                    {navbarLinks?.navbarList?.map((value: { href: To; label: string | number | boolean | ReactElement<any, string | JSXElementConstructor<any>> | Iterable<ReactNode> | null | undefined }, index: Key | null | undefined) => {
                      const isActive =
                        value.href ===
                          '/componentpage/ui-components/flowbite/buttons'
                          ? location.pathname.includes('/ui-components/')
                          : location.pathname === value.href
                      return (
                        <Link key={index} to={value?.href}>
                          <li
                            className={`relative h-[35px] overflow-hidden flex flex-col justify-start text-sm hover:bg-lightprimary dark:hover:bg-darkprimary  font-normal rounded-md group ${isActive
          ...
          <div
            className={`lg:hidden fixed top-0 right-0 h-full w-full bg-white dark:bg-black shadow-lg transform transition-transform duration-300 max-w-xs ${sidebarOpen ? 'translate-x-0' : 'translate-x-full'
              } z-50`}>
