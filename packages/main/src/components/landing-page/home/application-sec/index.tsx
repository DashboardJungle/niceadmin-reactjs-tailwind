...
import { applicationsVariation } from '../../data/templateData'
        ...
        <div className='container'>
          <div className='flex flex-col gap-7 lg:gap-12'>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5 lg:gap-8'>
              {applicationsVariation?.map((value, index) => {
                return (
                  <div
                    key={index}
                    className='flex flex-col gap-4 sm:gap-6 p-4 lg:p-6 bg-white dark:bg-black border border-forest-black/10 dark:border-white/10 rounded-2xl shadow-grid-shadow dark:shadow-dark-grid-shadow'>
                    <div className='border border-forest-black/10 rounded-lg'>
                      <img
                        src={value?.img}
                        alt={value?.title}
                        width={585}
                    ...
                    <div className='flex items-center justify-between'>
                      <div className='flex items-center gap-2'>
                        <h3 className='text-lg font-medium text-forest-black dark:text-darklink'>
                          {value?.title}
                        </h3>
                        <div
                          className={`${value?.tag == 'Free' ? 'bg-primary' : 'bg-secondary'
                            }  rounded-full`}>
                          <p
